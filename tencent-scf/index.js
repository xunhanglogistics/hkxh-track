...
async function callSdstTrack(mailNoList) {
  ...
  try {
    ...
    console.error('[scf/track] sdst selectTrack:', err.message || err);
    raw = null;
  }
  ...
  const text = await res.text();
  if (!res.ok) {
    throw new Error(`sdst trackIndex HTTP ${res.status}: ${text.slice(0, 200)}`);
  }
...
async function callNextslsTrack(mailNoList) {
  const code = mailNoList.map((s) => String(s).trim()).filter(Boolean)[0];
  if (!code) throw new Error('Empty tracking number');
  const qs = new URLSearchParams({ app: NEXTSLS_APP_ID, number: code });
  const url = `${NEXTSLS_LISTS_BASE}?${qs.toString()}`;
  const res = await fetch(url, {
    method: 'GET',
    headers: {
  ...
  const text = await res.text();
  if (!res.ok) {
    throw new Error(`NextSLS HTTP ${res.status}: ${text.slice(0, 160)}`);
  }
  let raw;
  try {
    raw = text ? JSON.parse(text) : {};
...
async function resolveAutoTrack(mailNoList, ctx) {
  ...
  const speedafP = (async () => {
    try {
      ...
      console.error('[scf/track] auto race speedaf:', err.message || err);
    ...
    handlers.push({
      ...
      start: () =>
        callNextslsTrack(mailNoList).catch((err) => {
          console.error('[scf/track] auto race nextsls:', err.message || err);
          return null;
        }),
    ...
    handlers.push({
      ...
      start: () =>
        callWawayTrack(mailNoList).catch((err) => {
          console.error('[scf/track] auto race waway:', err.message || err);
          return null;
        }),
    ...
    handlers.push({
      ...
      start: () =>
        callOisQueryTrace(mailNoList, 0, { isTranslateEn: oisEn }).catch((err) => {
          console.error('[scf/track] auto race ois:', err.message || err);
          return null;
        }),
    ...
    handlers.push({
      ...
      start: () =>
        callPortal218Track(mailNoList).catch((err) => {
          console.error('[scf/track] auto race portal218:', err.message || err);
          return null;
        }),
    ...
    handlers.push({
      ...
      start: () =>
        callYanwenTracking(mailNoList).catch((err) => {
          console.error('[scf/track] auto race yanwen:', err.message || err);
          return null;
        }),
    ...
    handlers.push({
      ...
      start: () =>
        callKingtransTrack(mailNoList).catch((err) => {
          console.error('[scf/track] auto race kingtrans:', err.message || err);
          return null;
        }),
    ...
    handlers.push({
      ...
      start: () =>
        callSz56tTrack(mailNoList).catch((err) => {
          console.error('[scf/track] auto race sz56t:', err.message || err);
          return null;
        }),
    ...
    handlers.push({
      ...
      start: () =>
        callWmsGetTrack(mailNoList).catch((err) => {
          console.error('[scf/track] auto race wms:', err.message || err);
          return null;
        }),
  ...
  return raceAutoTrackMergeByPriority(handlers, '[scf/track] auto race', (results) => {
    const s = results[0];
